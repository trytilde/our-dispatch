...
`tilde_list_memory_providers` → `tilde_create_memory_bank` (clear name and purpose) → save the id → verify with `tilde_get_memory_bank` and `tilde_check_memory_bank_health`. A bank changes the user's durable memory policy, so explain its purpose and visibility and confirm with the user before creating one.
