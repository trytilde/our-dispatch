...
4. When the user must choose or add an account, call your direct `configure_connector` tool with the provider's `tool_group_source_type_id`. The picker card renders for the user automatically: summarize why in one sentence and end your turn. Never paste authorization links or ask for credential values in chat.
...
- Enabling providers and adding accounts change the team's configuration: act on an explicit user request or picker selection; a picker click needs no extra confirmation.
- Reads (search, status) never need permission.
- With several accounts for one provider, say which one you use when it matters; show the picker instead of guessing.
