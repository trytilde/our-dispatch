...
- Replace the control service's per-session event proxy with Tilde's direct Mission Control WebSocket protocol.
...
flowchart LR
    ...
    T --> S[Direct Mission Control WebSocket]
...
- `packages/client-runtime` exchanges the owner session for a short-lived socket ticket, waits for one aggregate bootstrap on `mission_control.ready`, and remembers revisions only after an event is applied successfully.
...
- Control service
  - Add authenticated Mission Control ticket exchange, direct unified routine translation, and bulk connector binding; remove the `ws` dependency.
...
- `@trytilde/api-client` and `@trytilde/sdk`
  - Regenerate the OpenAPI surface for unified automations, Mission Control tickets, and provider-tool binding.
