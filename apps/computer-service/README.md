...
`ListCuaTools` and `CallCuaTool` expose the exact runtime Cua catalog and result envelope. One lazy private worker receives each agent's display, isolated home/XDG state (including Cua's browser data), and accessibility environment. Computer-service serializes catalog and action operations per agent display so simultaneous conversations cannot race the shared worker; different displays remain independent. Legacy screenshot and input RPCs are compatibility translations through Cua. noVNC remains a separate owner preview and takeover transport.
