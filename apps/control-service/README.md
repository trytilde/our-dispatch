...
The portable Hono control application. It serves health, exposes an allowlisted same-origin Tilde ChatKit REST/SSE bridge under `/api/chat/*`, bridges the server-authenticated team event socket to owner-authenticated SSE, and serves the built web UI with SPA fallback both locally and in a Vercel Function.
