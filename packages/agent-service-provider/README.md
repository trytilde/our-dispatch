...
Each agent must default-export `chatKitEndpoint(...)` from `agent.ts`. Global instrumentation runs before optional agent-local instrumentation and before the endpoint import. Standard agents must explicitly contain `bash.ts`, `await_shell.ts`, `read_file.ts`, `write_file.ts`, `copy_to_computer.ts`, `copy_from_computer.ts`, `glob.ts`, `grep.ts`, and `screenshot.ts`. Those Zod-schema Vercel AI SDK tools are imported by `agent.ts`, use the typed computer-service transport, and hide their fixed agent ID from model input; arbitrary authored tools and skills are not directory-loaded. An agent with `agent-profile.json` set to `computer-use-only` is the narrow exception: discovery requires both `tools/` and `skills/` to be absent, while `agent.ts` constructs its visual Computer-use surface directly.
