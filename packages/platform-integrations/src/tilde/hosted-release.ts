...
function allowedRuntimeName(name: string): boolean {
  return /^(?:TILDE_(?:API_KEY|ORG_ID|TEAM_ID|BASE_URL)|OPENBOT_(?:DEPLOYMENT_NAME|HOSTED_INSTANCE_ID|OIDC_[A-Z_]+)|COMPUTER_(?:SERVICE_API_KEY|SERVICE_URL|ID)|PUBLIC_ORIGIN|AGENT_SERVICE_ORIGIN|INFERENCE_PROVIDER|AI_MODEL|AGENT_[A-Z0-9_]+)$/.test(
    name,
  );
}
